    ...
    try {
      ...
      if (data?.user) {
        const { data: userData } = await supabase
          .from("usuarios")
          .select("nome, cidade, telefone, preferencias, peso, setup_completo")
          .eq("id", data.user.id)
          .single();
        ...
        if (conexoesAtivas && conexoesAtivas.length > 0) {
          ...
          if (conexaoRecente.status === "aguardando_assinatura") {
            if (!userData?.telefone || !userData?.peso) {
              navigation.reset({ index: 0, routes: [{ name: "MiniOnboarding", params: { conexaoId: conexaoRecente.id } }] });
              return;
            } else {
              navigation.reset({ index: 0, routes: [{ name: "PropostaAluno", params: { conexaoId: conexaoRecente.id } }] });
              return; 
            }
          }
        }
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={styles.innerContent}>
            ...
            <View style={styles.header}>
              ...
              <Text style={styles.subtitle}>Acesse sua conta para continuar sua jornada de treinos.</Text>
            </View>
            ...
            <View style={styles.form}>
              ...
              <TouchableOpacity style={[styles.btnOutline, loading && { opacity: 0.7 }]} onPress={handleLogin} disabled={loading} activeOpacity={0.8}>
                ...
                <Text style={styles.btnOutlineText}>{loading ? "Acessando..." : "Entrar na Conta"}</Text>
