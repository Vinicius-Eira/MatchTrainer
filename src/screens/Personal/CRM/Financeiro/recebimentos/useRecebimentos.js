...
export function useRecebimentos(navigation) {
  ...
  const [viewInicio, setViewInicio] = useState(
    new Date(hoje.getFullYear(), hoje.getMonth(), 1, 0, 0, 0),
  );
  const [viewFim, setViewFim] = useState(
  ...
  const carregarFinanceiro = useCallback(async () => {
    try {
      ...
      const dadosMapeados = (data || []).map((fat) => {
        ...
        const aluno = Array.isArray(plano.usuarios)
          ? plano.usuarios[0]
          : plano.usuarios || {};
        ...
        let detalheStatus =
          fat.tipo_receita === "avulsa" ? `Extra: ${fat.categoria_avulsa}` : "";
        ...
        if (fat.status === "pendente" && fat.tipo_receita !== "avulsa") {
          ...
          const diasDiferenca = Math.ceil(
            (dateObj.getTime() - hojeMeioDia.getTime()) / (1000 * 60 * 60 * 24),
          );
          ...
          detalheStatus =
            fat.tipo_receita === "avulsa"
              ? `Extra: ${fat.categoria_avulsa}`
              : "Pago";
        } else if (
          ["agendada", "cancelada", "congelada", "isenta"].includes(fat.status)
        ) {
          statusUi = fat.status;
          detalheStatus =
            fat.status.charAt(0).toUpperCase() + fat.status.slice(1);
        }
  ...
  const alunosFiltrados = mensalidades.filter((item) => {
    ...
    if (modalidadeAtiva === "Consultoria")
      passModalidade =
        !item.isHibrido && item.servicos_array?.includes("Consultoria");
    if (modalidadeAtiva === "Presencial")
      passModalidade =
        !item.isHibrido && item.servicos_array?.includes("Presencial");
    if (modalidadeAtiva === "Híbrido") passModalidade = item.isHibrido;

    let passStatus = true;
    if (statusAtivo === "Pagos") passStatus = item.status === "pago";
    if (statusAtivo === "Pendentes")
      passStatus =
  ...
  const alunosExtraFiltrados = alunosAtivos.filter((a) =>
    a.nome.toLowerCase().includes(buscaAlunoExtra.toLowerCase()),
  );
  const alunosCongelarFiltrados = alunosAtivos.filter((a) =>
    a.nome.toLowerCase().includes(buscaAlunoCongelar.toLowerCase()),
  );

  const totalRecebido = alunosFiltrados
    .filter((i) => i.status === "pago")
    .reduce((acc, curr) => acc + (Number(curr.valor_pago) || curr.valor), 0);
  const totalPendente = alunosFiltrados
    .filter(
      (i) =>
        i.status === "pendente" ||
        i.status === "proximo" ||
  ...
  const totalAtrasado = alunosFiltrados
  ...
  const textoHeaderData = () => {
    if (
      viewInicio.getDate() === 1 &&
      viewFim.getDate() ===
        new Date(
          viewInicio.getFullYear(),
          viewInicio.getMonth() + 1,
          0,
        ).getDate()
    ) {
      ...
      return `${viewInicio.getDate()}/${
        MESES_NOME[viewInicio.getMonth()]
      } - ${viewFim.getDate()}/${MESES_NOME[viewFim.getMonth()]}`;
    }
  };
  ...
  const setaMudarMesRapido = (direcao) => {
    ...
    setViewInicio(
      new Date(novaData.getFullYear(), novaData.getMonth(), 1, 0, 0, 0),
    );
    setViewFim(
      new Date(novaData.getFullYear(), novaData.getMonth() + 1, 0, 23, 59, 59),
    );
  };
  ...
  const gerarPdfDaTela = async () => {
    ...
    const filtroTexto =
      modalidadeAtiva !== "Todos" || statusAtivo !== "Todos"
        ? ` | Filtros: ${modalidadeAtiva} • ${statusAtivo}`
        : "";
    const html = `
      <html>
        <body style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; padding: 40px; color: #333;">
          ...
          <div style="display: flex; justify-content: space-between; margin-bottom: 30px;">
            <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; width: 30%; text-align: center; border: 1px solid #EEE;">
              <p style="margin:0; font-size: 12px; color: #666;">RECEBIDO</p><h2 style="margin:5px 0 0 0; color: #00E676;">R$ ${totalRecebido.toFixed(
                2,
              )}</h2>
            </div>
            <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; width: 30%; text-align: center; border: 1px solid #EEE;">
              <p style="margin:0; font-size: 12px; color: #666;">A RECEBER</p><h2 style="margin:5px 0 0 0; color: #333;">R$ ${totalPendente.toFixed(
                2,
              )}</h2>
            </div>
            <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; width: 30%; text-align: center; border: 1px solid #EEE;">
              <p style="margin:0; font-size: 12px; color: #666;">EM ATRASO</p><h2 style="margin:5px 0 0 0; color: #FF3B30;">R$ ${totalAtrasado.toFixed(
                2,
              )}</h2>
            </div>
          </div>
              ...
              ${alunosFiltrados
                ...
                <tr style="border-bottom: 1px solid #EEE;">
                  <td style="padding: 12px; font-weight: bold;">${
                    m.nome
                  }</td><td style="padding: 12px; color: #666;">${
                    m.isExtra ? m.categoria_avulsa : m.modalidadeUi
                  }</td>
                  ...
                  <td style="padding: 12px; font-weight: bold; color: ${
                    m.status === "pago"
                      ...
                      ? "#FF3B30"
                      : "#FFD700"
                  }">${m.status.toUpperCase()}</td>
                  <td style="padding: 12px; text-align: right;">R$ ${Number(
                    m.valor_pago || m.valor_cobrado,
                  ).toFixed(2)}</td>
                </tr>`,
                )
                .join("")}
  ...
  const confirmarBaixaManual = async () => {
    ...
    try {
      ...
      await supabase
        ...
        .insert([
          {
            personal_id: faturaSelecionada.planos.personal_id,
            aluno_id: faturaSelecionada.planos.aluno_id,
            referencia_id: faturaSelecionada.id,
            tipo_evento: "PAGAMENTO_RECEBIDO",
            descricao: `Pagamento de R$ ${valorFloat.toFixed(
              2,
            )} confirmado via ${formaPgto}.`,
            metadados: {
              valor: valorFloat,
              forma_pagamento: formaPgto,
              observacoes: observacao,
  ...
  const salvarReceitaExtra = async () => {
    if (!valorInput || !alunoSelecionadoId)
      return Alert.alert("Atenção", "Selecione o aluno e o valor.");
    setProcessando(true);
    try {
      const {
        data: { session },
      ...
      const { data: novaFatura, error: erroFatura } = await supabase
        .from("mensalidades")
        .insert([
          {
        ...
        .insert([
          {
            ...
            referencia_id: novaFatura.id,
            tipo_evento: "RECEITA_EXTRA_GERADA",
            descricao: `Venda de ${categoriaExtra} (R$ ${valorFloat.toFixed(
              2,
            )}) via ${formaPgto}.`,
            metadados: {
              valor: valorFloat,
  ...
  const confirmarCongelamento = async () => {
    if (!alunoSelecionadoId || !diasPausa)
      return Alert.alert("Atenção", "Preencha aluno e dias.");
    let motivoFinal = motivoPausa;
    if (motivoPausa === "Outro") {
      if (!textoOutroMotivo.trim())
        return Alert.alert("Atenção", "Por favor, detalhe o motivo da pausa.");
    ...
    try {
      ...
      await supabase
        ...
        .update({
          status: "congelado",
          observacoes: `[CONGELADO] Motivo: ${motivoFinal} | Retorno Previsto: ${dataRetornoCalculada}`,
        })
        ...
        .insert([
          {
            personal_id: session.user.id,
            aluno_id: alunoInfo.aluno_id,
            referencia_id: alunoSelecionadoId,
            tipo_evento: "CONTRATO_CONGELADO",
            descricao: `Plano congelado por ${diasPausa} dias (Motivo: ${motivoFinal}). Retorno previsto: ${dataRetornoCalculada}.`,
            metadados: {
              dias: diasPausa,
              motivo: motivoFinal,
              data_retorno_prevista: dataRetornoCalculada,
            },
          },
        ]);
      ...
      Alert.alert(
        "Sucesso",
        "Plano congelado com sucesso. O sistema pausará as próximas cobranças.",
      );
    } catch (e) {
      Alert.alert("Erro", "Falha ao congelar.");
    } finally {
      setProcessando(false);
    }
  };
  ...
  const abrirWhatsApp = (telefone, nome, valor, status) => {
    ...
    const msg =
      status === "atrasado"
        ? `Fala ${
            nome.split(" ")[0]
          }, tudo bem? Passando pra dar um toque sobre a mensalidade que venceu recentemente (R$ ${valor.toFixed(
            2,
          )}). Consegue ver isso hoje? 💪`
        : `Fala ${
            nome.split(" ")[0]
          }, tudo bem? Lembrete da renovação do nosso plano (R$ ${valor.toFixed(
            2,
          )}). Qualquer dúvida, avisa! 🚀`;
    Linking.openURL(
      `whatsapp://send?phone=55${num}&text=${encodeURIComponent(msg)}`,
    );
  };
  ...
  return {
    ...
    reciboRef,
    alunosFiltrados,
    alunosExtraFiltrados,
    alunosCongelarFiltrados,
    totalRecebido,
    totalPendente,
    totalAtrasado,
    totalGeral,
    ...
    salvarReceitaExtra,
    confirmarCongelamento,
    compartilharReciboImage,
    abrirWhatsApp,
    abrirModalBaixa,
    setTempAno,
    setModalDataVisivel,
    setFabAberto,
    setModalBaixaVisivel,
    setValorInput,
    setFormaPgto,
    setObservacao,
    setModalExtraVisivel,
    setBuscaAlunoExtra,
    setAlunoSelecionadoId,
    setCategoriaExtra,
    setModalCongelarVisivel,
    setBuscaAlunoCongelar,
    setMotivoPausa,
    setTextoOutroMotivo,
    setDiasPausa,
    onRefresh,
